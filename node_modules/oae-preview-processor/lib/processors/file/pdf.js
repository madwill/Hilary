...
var _pdf2swfBinary = null;
var _pdf2swfimeout = 120000;
...
var init = module.exports.init = function(config, callback) {
    ...
    _pdf2swfBinary = config.pdf2swf.binary;
...
var _convertPDFToHTMLPages = function(ctx, path, pagesDir, callback) {
    ...
    var endFile = pagesDir +'.swf';
    var cmd2 = util.format('"%s" "%s" -o "%s" -T 9 -t -G -s storeallcharacters', _pdf2swfBinary, path, endFile);

    log().info({'contentId': ctx.contentId}, 'Executing %s', cmd2);

    exec(cmd2, {'timeout': _pdf2swfimeout}, function (err, stdout, stderr) {
        if (err) {
            log().error({'err': err, 'stdout': stdout, 'stderr': stderr}, 'Could not convert pdf to SWF.');
            return callback({'code': 500, 'msg': 'Could not convert pdf to SWF'});
        }

        ctx.addPreview(endFile , 'swf');
